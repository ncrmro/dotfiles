...
validate_parent_chain() {
  ...
  for component in "${components[@]}"; do
    ...
    if [[ -n "${target_sources["${current}"]+present}" ]]; then
...
for package in "${selected[@]}"; do
  if [[ -n "${selected_packages["${package}"]+present}" ]]; then
...
for package in "${selected[@]}"; do
  ...
  while IFS= read -r -d '' source; do
    ...
    if [[ -n "${target_sources["${target}"]+present}" ]]; then
      printf 'Duplicate target: %s (%s and %s)\n' \
        "${target}" "${target_sources["${target}"]}" "${source}" >&2
      exit 1
    fi
